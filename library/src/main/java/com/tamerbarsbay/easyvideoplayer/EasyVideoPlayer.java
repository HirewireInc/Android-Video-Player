...
public class EasyVideoPlayer extends FrameLayout implements IUserMethods, TextureView.SurfaceTextureListener,
    ...
    public void setLeftAction(@LeftAction int action) {
        if (action < LEFT_ACTION_NONE || action > LEFT_ACTION_PREVIOUS)
    ...
    public void setRightAction(@RightAction int action) {
        if (action < RIGHT_ACTION_NONE || action > RIGHT_ACTION_NEXT)
...
//            if (!isPlaying()) start();
    ...
    private void setColors() {
        ...
        int colorControlsBg = Color.parseColor("#424242");
        mControlsFrame.setBackgroundColor(Util.adjustAlpha(colorControlsBg, 0.85f));
